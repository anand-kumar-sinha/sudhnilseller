...
const Category = ({ token }) => {
  ...
  const fetchCategory = async () => {
    try {
      const response = await axios.get(backandUrl + "/api/category/fetch");
  ...
  const removeProduct = async (id) => {
    try {
      const response = await axios.post(
        backandUrl + "/api/category/delete",
        { id },
        { headers: { token } }
      );
      if (response.data.success) {
  ...
  return (
    <>
      ...
      <p className="mb-2">All Category List</p>
